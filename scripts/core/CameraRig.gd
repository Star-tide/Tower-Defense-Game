...
func _ready() -> void:
	camera = $Camera3D
	camera_direction = camera.transform.origin.normalized()
	target_distance = camera.transform.origin.length()
	_apply_zoom()
...
func _apply_zoom() -> void:
	var new_position: Vector3 = camera_direction * target_distance
